...
% Updated by Nachon Raethong, 11-Dec-2024
...
% printFluxes(finalValidateModel, sol.x, true) 
run = finalValidateModel;
run.description = 'BCC7051 from iNR1329';
run.id = 'modelBCC7051fromiNR1329';


bccDraftFromCordyceps

exportForGit(finalValidateModel,'bccDraftFromCordyceps',...
    'C:\Users\Lenovo\Documents\GitHub\BCC7051-GSMM\model_drafts', {'xlsx'});
...
exportForGit(reduced5,'mergeModelsreduced5',...
    'C:\Users\Lenovo\Documents\GitHub\BCC7051-GSMM\model_drafts', {'xlsx'});

%% Import bcc7051 biomass
BCC7051biomass = importExcelModel('ComplementaryData/BCC7051_biomass4.xlsx');
...
goodModel22=setParam(adjModel,'ub',{'glcIN'},0.632);
goodModel22=setParam(goodModel22,'eq',{'BCC7051matp'},1);
goodModel22 = setParam(goodModel22,'obj',{'bmOUT'},1);

%goodModel22=setParam(adjModel,'ub',{'glcIN'},0);

sol = solveLP(goodModel22);
fprintf(['umax wt = ' num2str(sol.f*-1) ' per hour' '\n']);


modelSenerioII=setParam(goodModel22,'ub',{'glcIN'},0.5487);
...
fprintf(['umax mutant = ' num2str(solmodelSenerioII.f*-1) ' per hour' '\n']);

%% Improve grRules 
modelToImprove = goodModel22;
expression = '^or ';
replace = '';
...
solreducedModel = solveLP(reducedModel,1);
fprintf(['umax = ' num2str(solreducedModel.f*-1) ' per hour' '\n']);
%%
exportForGit(reducedModel,'newOryzae',...
    '/Users/nachonase/Documents/GitHub/BCC7051-GSMM/', {'xml','xlsx','mat','txt'});